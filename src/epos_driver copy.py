...
def print_center(string, width=60, fill_char='='):
    ...
    if (width - len(string)) % 2 != 0:
        print(f"{fill_char * fill_count} {string} {fill_char * (fill_count + 1)}")
    else:
        print(f"{fill_char * fill_count} {string} {fill_char * fill_count}")


class EPOS:
    def __init__(self, node_id:int =0, device_name:str="EPOS4", protocol_name:str="MAXON SERIAL V2", interface_name:str="USB"):
        try:
            current_dir = os.path.dirname(os.path.abspath(__file__))
            path = os.path.join(current_dir, "libEposCmd.so.6.8.1.0")
            print("Loading library from: %s" % path)
            self.epos = ctypes.cdll.LoadLibrary(path)
        except OSError as e:
        ...
        port_name = candidate_ports[self.node_id] if candidate_ports else None
        self.port_name = ctypes.c_char_p(port_name.encode('utf-8'))
        print(f"Port name: {self.port_name.value.decode('utf-8')}")
    ...
    def candidate_device(self):
        ...
        end_of_selection_port = ctypes.c_bool()
    ...
    def open_device(self):
        ...
        if self.key_handle != 0:
            print(f"Open Device_{self.node_id}, and key handle is: %8d" % self.key_handle)
            return True
    ...
    def close_device(self):
        ...
        if ret == 0:
            ...
            print(f"Device_{self.node_id} closed successfully!")
            return True
        ...
        if self.device_error_check():           # 检查设备是否有错误
            ...
            if ret == 0:
                ...
                if pls_enabled.value == 1:      # 验证设备是否使能
                    print(f"Device_{self.node_id} Enabled")
                    return True
                else:
                    print(f"Device_{self.node_id} Not Enabled!")
                    return False
        else:
    ...
    def disable_state(self):
        ...
        if ret == 0:
            ...
            if pls_disabled.value == 1:
                print(f"Device_{self.node_id} Disabled")
                return True
            else:
                print(f"Device_{self.node_id} could not be disabled")
                return False
    ...
    def ppm_move_position(self, target_angle:float, abosolute:bool=True, immediate:bool=True, block_time:int=10000):
        """电机移动到指定位置(Angle弧度单位)"""
        # 1. 角度转换为编码器单位
    ...
    def get_position(self, encoder_bool:bool=False):
        """获取电机位置(Encoder单位)"""
        ...
        if ret:
            if encoder_bool:
                ...
                position_angle = position_c.value * 2 * np.pi / (4 * 1024 * 35)
                print(f"Current Position: {position_angle}")
    ...
    def get_current(self):
        """获取电机电流"""
        current_is = ctypes.c_short()  # 用于存储实际电流值
        error_code = ctypes.c_uint()   # 用于存储错误代码
        ret = motor.epos.VCS_GetCurrentIs(motor.key_handle, motor.node_id, ctypes.byref(current_is), ctypes.byref(error_code))

        if ret:
            print(f"Device_{self.node_id} Current Value: {current_is.value}")
            return current_is.value
        else:
            print(f"VCS_GetCurrentIs failed!")
            print(f"Error Code: {error_code.value:#010x}; Description: {self.epos.VCS_GetErrorInfo(error_code.value)}")
            return False


if __name__ == "__main__":
    motor = EPOS(node_id=0)
...
try:
    ...
    motor.disable_state()
    motor.close_device()
    print_center("All Motors Disabled and Devices Closed!")